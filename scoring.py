...
def calculate_wallet_metrics(wallet: Dict, sells: List[Dict]) -> Dict:
    ...
    FIXED SCORING FORMULA (was broken):
      OLD: avg_profit (raw %) × 0.25  → 50% profit gives only 12.5 points
      NEW: normalize avg_profit to 0-100 first, then × 0.25
           → 50% profit gives (50/80)*100*0.25 = 15.6 points
           → 80% profit gives full 25 points (was 20)
    This unlocks wallets that were stuck just below threshold (e.g. 54.01 → 55+).
    """
    ...
    # === FIXED: normalize avg_profit to 0-100 before weighting ===
    # OLD (broken): min(avg_profit, MAX) * WEIGHT_AVG_PROFIT
    # NEW: (avg_profit / MAX) * 100 * WEIGHT_AVG_PROFIT
    norm_avg_profit = max(0.0, min(avg_profit / cfg.MAX_REASONABLE_PROFIT, 1.0)) * 100.0

    score = (
        win_rate * cfg.WEIGHT_WIN_RATE
        + norm_avg_profit * cfg.WEIGHT_AVG_PROFIT
...
def is_whale_candidate(wallet: Dict) -> bool:
    """
    A whale CANDIDATE: has ≥1 verified profitable sell, but not yet a full whale.
    Used for backfill priority + Telegram candidate alerts.
    """
    if (wallet.get("is_whale") or "").upper() == "TRUE":
        return False
    wins = _safe_int(wallet.get("winning_sells"))
    if wins < getattr(cfg, "WHALE_CANDIDATE_MIN_WINS", 1):
        return False
    # Must have a real sell with profit >= WHALE_CANDIDATE_MIN_PROFIT
    avg_profit = _safe_float(wallet.get("avg_profit"))
    if avg_profit < getattr(cfg, "WHALE_CANDIDATE_MIN_PROFIT", 5.0):
        return False
    return True


def get_whale_candidates(limit: int = 20) -> List[Dict]:
    """
    Return wallets that are whale candidates (have ≥1 winning sell but not yet whale),
    sorted by score descending. Used for backfill priority + weekly summary.
    """
    wallets = db.read_csv(cfg.WALLETS_FILE, db.wallet_headers())
    candidates = [w for w in wallets if is_whale_candidate(w)]
    candidates.sort(key=lambda w: _safe_float(w.get("score")), reverse=True)
    return candidates[:limit]


def get_new_candidates(prev_addrs: set) -> List[Dict]:
    """
    Given set of previously-alerted candidate addresses, return NEW candidates
    (just crossed the 1-win threshold this run).
    """
    candidates = get_whale_candidates(limit=100)
    return [c for c in candidates if c.get("address", "").lower() not in prev_addrs]
...
def promote_whales() -> List[Dict]:
    ...
    Also marks whales as 'dormant' if their wallet has been inactive too long
    (we keep them in whales.csv but mark status=dormant so they're not monitored).
    """
    wallets = db.read_csv(cfg.WALLETS_FILE, db.wallet_headers())
    existing = db.get_whale_addresses()
    newly = []
    promoted_now_set = set()  # for marking active
    ...
    for w in wallets:
        ...
        if not addr or addr in existing:
            ...
            if addr in existing:
                # Determine if wallet is still active (recent last_seen)
                last = w.get("last_seen") or ""
                status = "active"
                if last:
                    try:
                        ...
                        if age_days > cfg.WHALE_MAX_INACTIVE_DAYS:
                            status = "dormant"
                    except Exception:
                        pass
                db.upsert_whale({
    ...
    db.write_csv(cfg.WALLETS_FILE, db.wallet_headers(), wallets)
    active_count = len(db.get_whale_addresses())  # already filters to active
    logger.info("Whale promotion done. New: %d | Total active: %d", len(newly), active_count)
    return newly
